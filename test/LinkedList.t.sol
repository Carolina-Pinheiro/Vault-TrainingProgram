...
contract LinkedListTest is Test, ILinkedList {
    ...
    function testInsertFirstNode() external {
        ...
        Node memory node_ = Node({ nextId: nextId, endTime: endTime, shares: 10, dtAtDeposit: 0, isNewDeposit: true });
    ...
    function testInsertNewNode() external {
        ...
        Node memory newNode_ =
            Node({ nextId: nextId, endTime: endTime, shares: 10, dtAtDeposit: 0, isNewDeposit: true });
        ...
        newNode_ = Node({ nextId: nextId, endTime: endTime, shares: 20, dtAtDeposit: 0, isNewDeposit: false });
        list.insert(newNode_, previousId);
        ...
        endTime = endTime - 50;
        (previousId, nextId) = list.findPosition(endTime, list.getHead());
        newNode_ = Node({ nextId: nextId, endTime: endTime, shares: 20, dtAtDeposit: 0, isNewDeposit: true });
        list.insert(newNode_, previousId);
        assertEq(list.getHead(), 1); // the first node is the head
